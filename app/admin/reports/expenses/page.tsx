...
const TABS = [
  ...
  { key: 'vehicle-detail', label: '🚗 Vehicle Expenses' },
];
...
export default function ExpensesReportPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (tab !== 'monthly-trend') {
        if (dateFrom) params['from'] = dateFrom;
        if (dateTo)   params['to']   = dateTo;
      }
      const data = await reportApi.getSalesReport(`expenses/${tab}`, params);
      setReport(data);
    } catch {}
    setLoading(false);
  }, [tab, dateFrom, dateTo]);
    ...
    <AdminShell activeKey="reports">
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 8px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
          ...
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: t.text, margin: 0 }}>🧾 Expense Reports</h1>
            <p style={{ color: t.muted, fontSize: 13, margin: 0 }}>Category breakdown, trends, vehicle-linked costs</p>
          </div>
        </div>
        ...
        <ReportTable
          ...
          rows={report?.rows ?? []}
          loading={loading}
          exportEndpoint={`/reports/expenses/${tab}/export`}
